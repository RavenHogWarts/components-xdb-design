...
| `options.dayCount` | `1..6` | 否 | 仅 `days`，显示几天，默认 `3` |
| `options.weekStartsOn` | `'monday' \| 'sunday'` | 否 | 仅 `week` / `month`，周首日，默认 `'monday'` |
| `options.startField` | `string` | 是 | 事件开始日期字段名。缺省无事件 |
| `options.endField` | `string` | 否 | 事件结束日期字段名。缺省为单日事件 |

`startField` / `endField` 类型约束同 gantt：`date` / `datetime` 字段，或结果能被 moment 解析的 formula 字段（建议给公式字段写 `type: "date"` / `"datetime"`），也可用内置 `file.ctime` / `file.mtime` / `file.basename`。
...
calendar 支持 `filter` / `sort` / `limit` / `linkOpenMode`，不支持 `group` / `tree`。
