...
| `command`                | 是                 | 是          | 是           |
| `cform` / `templater`    | 是                 | 是          | 是           |
| `prompt`                 | 是                 | 是          | 是           |
| `script`                 | 是                 | 是          | 是           |

表格只列内置 type。`*.xdb.js` 插件可以通过 `registerAction()` 注册其它 type，由对应插件的 `match` 决定在哪些入口可选：
...
- 插件未安装、已卸载或 scope 不匹配时，配置仍会保留，但编辑器显示不支持，执行时报 `Unsupported action: <type>`。
