...
type DatabaseViewDefinition = {
  ...
  /** 当前 view 的过滤条件，结构见 types/database.md#filteritem */
  filter?: FilterItem;
  ...
  /** 新建记录动作（内置或 registerAction 注册的 type，仅 file 源有效） */
  newRowAction?: Action;
  /** 行链接 / 内链的打开方式，默认 'tab' */
...
> 公共上下文本身的完整 API（尤其 `dailyNotes` / `markdown` / `files` / `tasks`）统一见 [types.md 的「公共上下文」](types.md#公共上下文xdbcontextprops)，这里不重复展开。

```ts
type ViewProps = XdbContextProps & {
  /** 当前 view 的挂载容器 */
  container: HTMLElement;
  /** 数据库读写入口（updateView/updateRow/updateCell/getRowLink…），完整能力见 types/database.md */
  api: Database;
  viewId: string;
...
type DatabaseViewProps = ViewProps & {
  viewData: {
    ...
    groups: Array<{
      ...
      rows: Array<{
        id: string;
        /** 原始行数据，结构见 types/database.md 的「行数据 $item」 */
...
type DatabaseFieldDefinition = {
  name: string;
  type?: DatabaseFieldType; // 'text' | 'number' | 'boolean' | 'date' | 'datetime' | 'select' | 'multi-select' | 'reference' | 'button' | string
  formula?: string;
  options?: Record<string, unknown>;
};
```
...
通过 `registerDatabaseView()` 注册的视图会**自动**获得四个内置设置 tab：`field` / `filter` / `sort` / `group`，与 table / gallery / list / kanban / calendar / gantt 等内置视图完全一致，无需手动注册。这些 tab 直接读写 `viewDefinition.visibleFields` / `filter` / `sort` / `group`，数据源仍是当前视图。

- `registerView()` 注册的普通视图**不会**获得这些 tab（charts / metric / group / markdown / reference / button 均属此类）。
- 如需追加自定义 tab，用 [`registerViewSettingsTab()`](view-settings-tab.md) 注册，自动 tab 与自定义 tab 并存。
- 如需向共享的 `View` tab 注入视图专属设置项，用 [`registerViewSettings()`](view-settings.md)。
