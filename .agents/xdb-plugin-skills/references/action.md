...
- 第三方编辑器使用 `editor()`；`editorComponent` 是内置 React 实现专用的 `@internal` API。
...
builder 的完整方法列表（`switch / input / numberInput / select / picker / combobox / autocomplete / fileAutocomplete / fileCombobox / folderAutocomplete / folderCombobox / propertyAutocomplete / propertyCombobox / fieldAutocomplete / action / popover / title / description / divider / custom`）与 view settings 共用同一套 API，详见 [view-settings 的「复用宿主 setting 组件」](view-settings.md#复用宿主-setting-组件propssetting)。
...
```bash
node docs/skills/xdb-plugin-skills/scripts/validate-xdb-plugin.mjs path/to/plugin.xdb.js
```
