...
工具栏里出现哪些按钮、它们的可见/激活/禁用状态以及点击行为；它不渲染视图主体，也不读行数据。
...
type ViewActionMenuResult =
  | void
  | Readonly<{ type: 'created-item'; href: string; status: 'visible' | 'not-visible' }>;

type ViewActionMenuExtension = {
  /** 稳定身份；同一 registry 内唯一。 */
  id: string;
  label: string;
  /** camelCase Lucide 名称，例如 search。 */
  icon: string;
  ...
  | { settingTabId?: never; onClick: (context: ViewActionMenuContext) => ViewActionMenuResult | Promise<ViewActionMenuResult> }
);
```
...
- View Action Menu **不依赖** `registerView()` / `registerDatabaseView()`：你可以为任意视图类型（包括内置 table / kanban / calendar）追加菜单项，只要 `viewTypes` 命中。
- 用 `settingTabId` 打开的 settings tab，应已由 `registerViewSettingsTab()` 或宿主内置提供；否则点击不会切换到任何面板。
- View Action Menu 不读行数据；需要行级操作的入口用按钮字段或 Button View（见 [action](action.md)）。
