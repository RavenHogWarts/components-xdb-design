...
| 组头自由表达式    | `group.summary.expression`        | 聚合             | `$items`、`$values`、`moment`                     | 显示在组头的值     |
| 表格列自由表达式  | `view.summary[field].expression`  | 聚合             | `$items`、`$values`、`moment`                     | 显示在表格底部的值 |
| metric 自由表达式 | `metric.aggregate.expression`     | 聚合             | `$items`、`$values`、`moment`                     | 单个统计值         |
| charts 自由表达式 | `measures[].aggregate.expression` | 聚合             | `$items`、`$values`、`moment`                     | 对应值轴类型的值   |
...
聚合配置优先使用内置计算。只有配置的 `type` 是 `expression` 时才直接写本节表达式；对象结构和内置计算见 [aggregate.md](aggregate.md)。
...
- `group.summary.expression`
- `view.summary[field].expression`
- `metric.aggregate.expression`
- `charts.measures[].aggregate.expression`

可用变量：

- `$items`：当前这组行的对象数组
- `$values`：
  - 在 group / metric / charts 的自由表达式里，等同于 `$items`
...
| 看“本月总支出”                  | `metric.aggregate`，优先 `{ type: "sum", field: "amount" }` |
| 表格底部统计“总金额”            | `view.summary.price`                                        |
| 图表按月显示“营收”              | `charts.measures[].aggregate`，优先内置计算                 |
...
- formula 和 `{ type: "expression" }` 聚合都写成一个可返回值的表达式；不要在末尾加分号。
...
- 筛选表达式：[filter.md#expression-dsl](filter.md#expression-dsl)
- 组头汇总：[group.md#summary](group.md#summary)
- 聚合配置：[aggregate.md](aggregate.md)
