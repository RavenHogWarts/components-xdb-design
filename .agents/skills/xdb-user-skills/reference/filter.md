...
- 每个节点的 `id` 都必须是非空字符串，并在当前 filter tree 内唯一；编辑已有节点时保持 id 稳定。
- `group.items` 可为空；空 group 不排除任何行。`join` 仍必须写 `and` 或 `or`。
- 空白 `expression` 不排除任何行，但生成配置时应删除空规则，避免留下无意义节点。
...
先跑可移植的结构与高置信语法检查（`<skill-dir>` 即本 skill 文件夹）：
