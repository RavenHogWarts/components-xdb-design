...
function validateViewLike(ext, label) {
  ...
  if (ext.viewComponent != null)
    return { ok: false, reason: `${label}（${ext.id}）：viewComponent 是 @internal；第三方使用 view()` };
  if (typeof ext.view !== 'function')
...
function validateSettingsLike(ext, label) {
  ...
  if (ext.viewTypes != null) {
    ...
    if (ext.viewTypes.some((t) => !isNonBlankString(t)))
      return { ok: false, reason: `${label}（${ext.id}）：viewTypes 的每一项都需为非空字符串` };
  }
  if (ext.settingsComponent != null)
    return { ok: false, reason: `${label}（${ext.id}）：settingsComponent 是 @internal；第三方使用 settings()` };
  if (typeof ext.settings !== 'function')
...
function validateAction(ext) {
  ...
  if (!isNonBlankString(ext.icon)) return { ok: false, reason: `${label}（${ext.type}）：缺少合法 icon` };
  if (ext.description != null && typeof ext.description !== 'string') {
    return { ok: false, reason: `${label}（${ext.type}）：description 需为字符串` };
  }
...
function validateViewActionMenu(ext) {
  ...
  if (ext.order != null && !Number.isFinite(ext.order))
    return { ok: false, reason: `${label}（${ext.id}）：order 需为有限数字` };
  if (ext.viewTypes != null && (!Array.isArray(ext.viewTypes) || ext.viewTypes.some((t) => !isNonBlankString(t)))) {
    return { ok: false, reason: `${label}（${ext.id}）：viewTypes 的每一项都需为非空字符串` };
  }
