...
先读 [Field Type API Schema](api-schema.md#field-type)。本页说明 Field Type 与 renderer/settings/source capability 的组合边界。
...
type DatabaseFieldTypeExtension = {
  ...
  /** picker 显示名；必须是非空字符串。 */
  label: string;
  /** 非空 Lucide 名称；推荐 camelCase，例如 star；未知名称回退为 Text。 */
  icon: string;
  /** 有限数；越小越靠前，省略为 0。 */
  order?: number;
  /**
...
`registerFieldType()` 返回 `boolean`。`type` / `label` / `icon` 为空、`order` 非有限数、`match` 不是函数或 `type` 重复时返回 `false`；插件不应继续假设类型已经安装：
