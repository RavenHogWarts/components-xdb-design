...
完整 View 外壳见 [`.xdb` Schema](schema.md#databaseviewdefinition)，各 View 的组合方式见 [View Schema](view-schema.md)。这里仅定义共享卡片字段。
...
interface CardCoverOptions {
  ...
  coverObjectFit?: string; // CSS object-fit；内置设置提供 'cover' / 'contain'
...
| `coverObjectFit`   | `string`  | 否   | CSS `object-fit`；默认 `'cover'`，内置设置提供 `'cover'` / `'contain'` |
| `extensionData`    | `object`  | 否   | 当前封面来源的私有配置，随 `cover` 而变                                |
| `hideFieldName`    | `boolean` | 否   | 卡片上隐藏字段名只显示值，默认 `true`；显式 `false` 才显示字段名       |
