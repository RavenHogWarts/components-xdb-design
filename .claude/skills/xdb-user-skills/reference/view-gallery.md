...
完整定义以 [View Schema](view-schema.md#gallery) 为准。这里说明 Gallery 的卡片用法。
...
| 字段                    | 类型      | 必填 | 说明                                                                                                 |
| ----------------------- | --------- | ---- | ---------------------------------------------------------------------------------------------------- |
| `options.cardSize`      | `number`  | 否   | 卡片目标宽度（像素），100–600，默认 `220`。列数自动计算（响应式，非固定列数）                        |
| `options.hideFieldName` | `boolean` | 否   | 是否隐藏字段名只显示值，默认 `true`；要显示字段名需显式写 `false`                                    |
