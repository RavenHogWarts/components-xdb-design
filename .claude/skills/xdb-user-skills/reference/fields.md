...
字段（属性）是数据库的列定义，全局共享，所有数据 View 使用同一套字段。外层结构见 [`.xdb` Schema](schema.md#databasefielddefinition)；本页定义内置 Field type 的值和 `options`。
...
type DatabaseFieldType =
  ...
  | 'formula'
  | 'button'
  | string; // 已安装插件注册的 Field type id
```

| 字段      | 类型                | 必填 | 说明                                                                   |
| --------- | ------------------- | ---- | ---------------------------------------------------------------------- |
| `name`    | `string`            | 是   | 属性名，在 `fields` 中唯一                                             |
| `type`    | `DatabaseFieldType` | 否   | Field type id，默认 `text`；开放字符串，第三方 type 必须来自已安装插件 |
| `options` | `object`            | 否   | 当前 Field type 拥有的配置；修改时保留未知 sibling key                 |
| `formula` | `string`            | 否   | 公式表达式；存在时字段只读，见 [formula](#formula)                     |
...
`formula` 是当前可选的内置 type，新配置写成 `{ "type": "formula", "formula": "..." }`。为了兼容已有配置，只要 `formula` 字段存在，即使 `type` 是 `number` / `date` 等也仍按只读公式处理。

没有 `status` 类型。任务库的“状态”是 `text`（值为状态字符）；文件库的“状态”通常是 `select`。

`type` 是开放字符串。已安装插件可以注册其它 Field type，并拥有该字段的 `options` schema；没有插件 schema 就不要生成。修改已有未知 type 时保留 `type` 和完整 `options`。
...
| `color` | 可选；省略时按 `value` 自动配色。显式颜色枚举：`default` / `gray` / `orange` / `yellow` / `green` / `teal` / `blue` / `purple` / `pink` / `red` |
...
内置支持更新/删除属性、移动/删除记录、创建/打开文件、命令、集成、Prompt 和 Script；也可使用插件注册并支持 `button-field` 上下文的自定义 Action。插件的 `match` 只控制 picker。完整 schema、入口能力矩阵、Script bindings，以及移动/重命名后的 `$item.file.path` 语义见 [actions.md](actions.md)。不再使用 `options.steps`、`inline-formula` 或直接操作 Obsidian FileManager 的动作函数。

## formula

`type: "formula"` 或存在 `formula` 的字段都是只读计算字段，其值由一段表达式算出。新配置使用 `type: "formula"`：

```json
{ "name": "总价", "type": "formula", "formula": "price * quantity" }
```

formula 字段**只读**，不能编辑。Calendar 和 Gantt 接受 `formula` 字段作为时间字段，但表达式结果仍必须能被日期解析。先理解几类表达式的区别，可看 [expressions.md](expressions.md)。
...
> `group.summary` 使用统一聚合配置，自由表达式分支在聚合上下文中求值（见 [aggregate.md](aggregate.md)）。筛选 `expression` 使用单独的动态公式 DSL，只执行白名单 Operation，不执行 JavaScript（见 [filter.md#expression-dsl](filter.md#expression-dsl)）。
