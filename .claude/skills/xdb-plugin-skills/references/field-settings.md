...
先读 [Field Settings API Schema](api-schema.md#field-settings) 和 [Setting UI Schema](types/setting-ui.md)。本页说明匹配、配置归属和设置写回。
...
Field Settings 与 View Settings、Action editor 复用同一个 `SettingUi`。完整方法、参数字段、返回 handle 与
cleanup 见 [Setting UI Schema](types/setting-ui.md)。
...
完整的 Field Type + Renderer + Settings 组合示例见 [field-type.md 的「独立组合」](field-type.md#独立组合)。
