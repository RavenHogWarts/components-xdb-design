...
先读 [Card Cover API Schema](api-schema.md#card-cover)。本页说明配置位置与实现方式。
...
> 公共上下文字段见 [公共 API Schema](api-schema.md#公共运行时上下文)；helper 方法由 [types.md](types.md) 路由。如果封面逻辑需要读 markdown、找日记、移动文件，优先看 `props.markdown`、`props.dailyNotes`、`props.files`。
...
type DatabaseViewCoverProps = XdbContextProps & {
  ...
  /** 写私有配置 */
  updateData: (
    update: Record<string, unknown> | ((current: Readonly<Record<string, unknown>>) => Record<string, unknown>)
  ) => void;
  /** 当前卡片对应的原始行数据 */
